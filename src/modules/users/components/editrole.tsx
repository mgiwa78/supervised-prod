...
const EditRole = ({
  ...
  const handleChange = (e: any, id: any) => {
    if (formik.values.status[id]) {
      ...
      formik.values.status[id] = {state: true, id}
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, {setSubmitting}) => {
      ...
      console.log(values)
      try {
        await put('roles', values, token, true, 'Role Updated Successfully').then(() => {
          setSubmitting(false)
          handleClose()
        })
      } catch (ex) {
                            ...
                            <td>
                              <label className='form-check form-check-custom form-check-solid me-9'>
                                <input
                                  className='form-check-input'
                                  type='checkbox'
                                  defaultChecked={status[perm._id] ? status[perm._id].state : false}
                                  name={perm._id}
